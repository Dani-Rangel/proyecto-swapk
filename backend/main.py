...
app = FastAPI(
    title="SWAPK Backend",
    description="API para el sistema de trueques y cursos",
    version="1.0.0"
)
...
origins = [
    "http://localhost:3000",
    "https://victorious-empathy-production-50dd.up.railway.app",
    "https://*.railway.app",
]
...
def get_all_public_profiles(page: int = 1, limit: int = 12, db: Session = Depends(get_db)):
    if page < 1: page = 1
    if limit < 1: limit = 12
    if limit > 100: limit = 100
    offset = (page - 1) * limit
    total = db.query(Perfil).join(Usuario).count()
    perfiles = db.query(Perfil).join(Usuario).offset(offset).limit(limit).all()
    resultado = [{
        "id": p.id,
...
# 🔥 Lanzador con migraciones automáticas
if __name__ == "__main__":
    ...
    print(f"🚀 Iniciando backend en http://0.0.0.0:{port}")
