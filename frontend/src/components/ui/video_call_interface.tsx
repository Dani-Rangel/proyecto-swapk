...
export default function VideoCallInterface() {
  ...
  useEffect(() => {
    ...
    } else if (webRtcCallState.isCalling) {
      setViewMode("calling")
    } else {
  ...
  }, [webRtcCallState.isInCall, webRtcCallState.isCalling])
  ...
  useEffect(() => {
    if (webRtcCallState.isInCall) {
      const timer = setInterval(() => {
        setNotification(`En llamada con ${webRtcCallState.currentContact}`)
      }, 10000)
  ...
  const handleCallAction = (action: string) => {
    ...
    if (action === "hang-up" || action === "decline") {
      setViewMode("chat")
    }
  }
        ...
        <div className="w-full max-w-5xl h-[calc(100vh-180px)] bg-[#141414] rounded-2xl overflow-hidden shadow-2xl flex flex-col border border-gray-700/30">
          <div className="flex-1 relative group">
            {webRtcCallState.remoteStream ? (
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-[#141414]">
                <div className="text-center space-y-4">
                  ...
                  <p className="text-gray-400 text-sm">Esperando video de {webRtcCallState.currentContact}...</p>
                </div>
              </div>
            )}
            ...
            <div className="absolute top-6 left-6 bg-blue-500/70 backdrop-blur-md px-4 py-2.5 rounded-full text-sm font-medium flex items-center gap-3 border border-blue-400/40 shadow-lg">
              <img
                src={`https://ui-avatars.com/api/?name=${encodeURIComponent(webRtcCallState.currentContact)}&background=random&size=32`}
                alt={webRtcCallState.currentContact}
                className="w-8 h-8 rounded-full border border-white/40"
              />
              <span className="text-white">{webRtcCallState.currentContact}</span>
            </div>
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-[#1a1a1a] p-6">
        <div className="bg-gray-900 rounded-2xl p-12 text-center max-w-md w-full border border-gray-700/50 shadow-2xl">
          ...
          <h2 className="text-3xl font-bold text-white mb-3">Llamando a {webRtcCallState.currentContact}...</h2>
  ...
  return (
    <div className="flex h-screen bg-[#141414] text-white">
      ...
      <div className="flex-1 flex flex-col relative">
        {selectedContact && !webRtcCallState.isInCall && !webRtcCallState.isCalling && (
        ...
        <div className="flex-1 relative">
          ...
          {webRtcCallState.isCalling && renderCallingView()}
          {!webRtcCallState.isInCall && !webRtcCallState.isCalling && viewMode === "chat" && (
            <ChatArea
              currentContact={selectedContact}
              onContactSelect={handleContactSelect}
              onViewChange={setViewMode}
