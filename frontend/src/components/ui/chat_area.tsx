...
export function ChatArea({
    ...
    const fetchUsers = async () => {
      ...
      try {
        ...
        const usersFromApi = data
          .filter((user) => String(user.id) !== myUserId)
          .map((user) => ({
            id: String(user.id),
            name: user.name,
  ...
  useEffect(() => {
    const fetchUserChats = async () => {
      ...
      try {
        ...
        if (data.chats?.length > 0 && !currentContact && onContactSelect) {
          ...
          const otherUser = firstChat.usuarios.find((u: ChatUsuario) => String(u.id) !== String(myUserId));
          if (otherUser) {
    ...
    fetchUserChats();
  }, [token, myUserId, onContactSelect, API_URL, currentContact]);
  ...
  useEffect(() => {
    ...
    const findUserInChats = (): User | null => {
      for (const chat of userChats) {
        const otherUser = chat.usuarios.find(
          (u: ChatUsuario) => String(u.id) !== String(myUserId) && u.nombre === currentContact
        );
        if (otherUser) {
          return {
    ...
    const initializeChat = async () => {
      try {
        ...
        const data: { chat_id: number } = await res.json();
        ...
        if (!messagesRes.ok) {
          const errorData: { detail?: string } = await messagesRes.json().catch(() => ({}));
          throw new Error(errorData.detail || `Error ${messagesRes.status}`);
        }

        const messagesData: MessagesResponse = await messagesRes.json();
        const validMessages = messagesData.messages.filter((msg) => msg.contenido);

        type MessageType = "text" | "file";

        const loadedMessages = validMessages.map((msg) => {
          ...
          const getMessageType = (tipo: string): MessageType => {
            if (tipo === "imagen" || tipo === "archivo" || tipo === "file") {
              return "file";
            }
            return "text";
          };
        ...
        websocket.onmessage = (event) => {
          ...
          if (data.type === "message") {
            ...
            // Aca verificamos si esos mensajes pertencen al CHAT ACTUAL
  ...
  }, [currentContact, allUsers, userChats, myUserId, token, API_URL, ws]);
  ...
  const handleEditMessage = async (messageId: string, newContent: string) => {
    ...
    try {
      ...
      const updatedMsg: { contenido: string; fecha: string } = await res.json();
        ...
        {userChats.length > 0 ? (
          <div className="w-full max-w-md">
            ...
            <div className="space-y-3">
              {userChats.map((chat) => {
                const otherUser = chat.usuarios.find((u: ChatUsuario) => String(u.id) !== String(myUserId));
